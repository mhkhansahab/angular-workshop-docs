...
To manage data in our Angular application, we will create an Angular Service because this is a prominent practice to keep the HTTP calls or State Management inside Angular Services.
...
## Let's write the interface first

We will create an `interface` for the data type we're going to use. I.e. for products. Let's create a new file inside our `src/app/shared/interfaces` folder and name it `product.interface.ts`. Then add the following code to it:

<Tabs>
  <TabItem label="src/app/shared/interfaces/product.interface.ts">
  ```ts
...
## Create first Signal inside Product Service

This creates the `Product Service` in the above mentioned folder. Now, let's create our first signal that will handle the products state:

<Tabs>
  <TabItem label="src/app/shared/services/product.service.ts">
  ```ts
...
export class ProductService {
     ...
     *
     * This is a signal that returns the selected products as a readonly array.
     * The selected products are the products that are currently filtered by the selected category.
     *
     * @returns A readonly signal of the selected products.
     */
    getSelectedProducts() {
...
## Injecting the service, and getting the data

Now we can use the service to get the data, inside the 'app.component.ts' file.
